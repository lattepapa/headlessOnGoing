...
import { title, remove, variant, totals, priceColumn } from './line-item.module.css'

function LineItem({ item }: any) {
    const { removeLineItem, checkout, updateLineItem, loading }: any = React.useContext(StoreContext)
    ...
    return (
        <tr>
            <td>{image && <GatsbyImage key={variantImage.src} image={image} alt={variantImage.altText ?? item.variant.title} />}</td>
            <td>
                <h2 className={title}>{item.title}</h2>
                <div className={variant}>{item.variant.title === 'Default Title' ? '' : item.variant.title}</div>
