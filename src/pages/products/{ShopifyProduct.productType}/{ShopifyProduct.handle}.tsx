...
export default function Product({ data: { product, suggestions } }: any) {
    const {
        options,
        variants,
    ...
    console.log(images)
    const { client }: any = React.useContext(StoreContext)

    const [variant, setVariant] = React.useState<any>({ ...initialVariant })
    const [quantity, setQuantity] = React.useState<number>(1)

    const productVariant: any = client.product.helpers.variantForOptions(product, variant) || variant
    ...
    const checkAvailablity = React.useCallback(
        (productId) => {
            client.product.fetch(productId).then((fetchedProduct: any) => {
                const result = fetchedProduct?.variants.filter((variant: any) => variant.id === productVariant.storefrontId) ?? []
            ...
            <div className={container}>
                <div className={productBox}>
                            ...
                            <div role="group" aria-label="gallery" aria-describedby="instructions">
                                <ul className={productImageList}>
                                    {images.map((image: any, index: any) => (
                                        <li key={`product-image-${image.id}`} className={productImageListItem}>
                                            <GatsbyImage
                                                objectFit="contain"
                    ...
                    <div>
                        ...
                        <div>{ReactHtmlParser(descriptionHtml)}</div>
